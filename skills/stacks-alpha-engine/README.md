...
**HODLMM integration?** Yes: 16 pools scanned with YTG ratios
...
The 16 HODLMM pools it knows are scanned with YTG ratios per pool. Reads user positions via `get-user-bins`, `get-overall-balance`, `get-active-bin-id`. Calculates break prices via DLMM Core `get-bin-price`. Generates `add-liquidity-simple` and `withdraw-liquidity-simple` instructions.
...
*The submission text as it was written (April 2026). Counts in it describe the skill at that time: it now scans 9 tokens and 16 HODLMM pools.*
...
The 16 HODLMM pools it knows are scanned with YTG ratios per pool. Reads user positions via `get-user-bins`, `get-overall-balance`, `get-active-bin-id`. Calculates break prices via DLMM Core `get-bin-price`. Generates `add-liquidity-simple` and `withdraw-liquidity-simple` instructions. Rebalance with 4h cooldown.
