...
**HODLMM integration?** Yes: scans 12 HODLMM pools
...
2. **Available ZBG Positions**: Active deposits across all 3 protocols + 12 HODLMM pools
...
Scans 12 HODLMM pools for user positions, active bins, and APR. Shows per-pool yield in the Smart Options table. Break prices show where HODLMM bins exit range.
...
**Category:** Yield
**HODLMM integration?** Yes: scans 12 HODLMM pools, reads bin positions, calculates break prices from on-chain bin math
...
2. **Available ZBG Positions** - Checks all three protocols for active deposits. Scans 12 HODLMM pools. If nothing deployed: "idle, earning nothing."
...
- **Direct on-chain reads for protocol state.** Granite params and HODLMM bin positions are read directly from on-chain contracts via Hiro `call_read_only_function`, no third-party aggregator. HODLMM APR and TVL come from Bitflow API (first-party, same team that deploys the pools). Zest positions and rates are read on-chain from the Zest V2 contracts, with no fallback: a failed read reports the position as unknown.
...
- **Zest rates are read per coin, and a coin that pays little shows little.** Each vault's supply rate comes from its own `get-interest-rate`, `get-utilization` and `get-fee-reserve`. Lenders earn only while people borrow, so sBTC read 0.001% on 2026-09-13. A rate that could not be read is left out of the ranking, never shown as 0%. Until 2026-09-13 this row was a typed 0% with a note to check zest.fi, and the position read pointed at a contract that does not exist; the example output above predates that fix.
